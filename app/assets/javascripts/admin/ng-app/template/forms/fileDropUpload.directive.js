        ...
        function($timeout, Upload){
          return {
            ...
            template : '\
                      ...
                      <div ng-class="loading_class()" class="user-avatar"\
                        ngf-size="thumbnail()"\
                        ngf-thumbnail="item[field] || item[thumbField]"\
          ...
          function controller($scope, $element){
            ...
            $scope.upload = function (file, ev) {
              ...
              if (!file.$error) {
                var upload_conf = {
                  url: '/admin/api/' + $scope.model.model_slug + "/" + $scope.item.id,
                  method: 'PUT',
                  data: {}
                };
                ...
                Upload.upload(upload_conf)
                    ...
                    .success(function (data, status, headers, config) {
                      $timeout(function() {
                        $scope.loading = false;
                        $scope.item[$scope.thumbField] = data[$scope.thumbField];
                      });
                    });
              }
            };
